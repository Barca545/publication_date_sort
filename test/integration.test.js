...
// Add a test for fetching and then sorting
// take expected from the last one but use all the fetch stuff as normal

test("Fetch, Parse, & Sort", (_t) => {
  let titles = getAppearances("Lian Harper (Prime Earth)");
  let src = getAppearancePages(titles);

  // Neeed a function to convert source into a ListEntry[]
  // I think I should pull out the function currently in load into a helper function
